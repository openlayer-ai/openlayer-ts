...
import { APIPromise } from 'openlayer/core/api-promise';

import util from 'node:util';
import Openlayer from 'openlayer';
import { APIUserAbortError } from 'openlayer';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['OPENLAYER_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Openlayer) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Openlayer({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Openlayer({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['OPENLAYER_LOG'] = 'debug';
      const client = new Openlayer({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['OPENLAYER_LOG'] = 'not a log level';
      const client = new Openlayer({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'OPENLAYER_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['OPENLAYER_LOG'] = 'debug';
      const client = new Openlayer({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['OPENLAYER_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Openlayer({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['OPENLAYER_BASE_URL'] = 'https://example.com/from_env';
      const client = new Openlayer({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['OPENLAYER_BASE_URL'] = ''; // empty
      const client = new Openlayer({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.openlayer.com/v1');
    });

    test('blank env variable', () => {
      process.env['OPENLAYER_BASE_URL'] = '  '; // blank
      const client = new Openlayer({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.openlayer.com/v1');
    });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Openlayer({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Openlayer({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['OPENLAYER_API_KEY'] = 'My API Key';
    const client = new Openlayer();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['OPENLAYER_API_KEY'] = 'another My API Key';
    const client = new Openlayer({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Openlayer({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Openlayer({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Openlayer({ apiKey: 'My API Key', fetch: testFetch });
