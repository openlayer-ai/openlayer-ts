...
export class Sections extends APIResource {
   ...
   * ```ts
   * const response =
   *   await client.governance.frameworks.sections.listRules(
  ...
  ): APIPromise<SectionListRulesResponse> {
    const { frameworkId, ...query } = params;
    return this._client.get(path`/frameworks/${frameworkId}/sections/${sectionID}/rules`, {
      query,
      ...options,
    });
  }
...
export interface SectionListRulesResponse {
  items: Array<SectionListRulesResponse.Item>;
}
...
export interface SectionListRulesParams {
  /**
   ...
   * subsections.
   */
  includeSubsectionRules?: boolean;
