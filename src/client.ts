...
export class Openlayer {
   ...
   */
  constructor({
    baseURL = readEnv('OPENLAYER_BASE_URL'),
    apiKey = readEnv('OPENLAYER_API_KEY') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? Openlayer.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('OPENLAYER_LOG'), "process.env['OPENLAYER_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('OPENLAYER_CUSTOM_HEADERS');
    if (customHeadersEnv) {
